...
class TestEnvironment(unittest.TestCase):
    ...
    def test_step(self):
        ...
        def checkStep(env):
            ...
            lastState = env.reset()
            self.assertTrue(env.observation_space.low.shape == lastState.shape, "State incoherent with the space defined")
            for _ in xrange(20):
                action = env.action_space.sample()
                state, r, done, _ = env.step(action)
                
                self.assertTrue( lastState.shape == state.shape, "Previous state has different shape by the actual: " + str(lastState) + " vs " + str(state))
                self.assertTrue( type(r) in set([float,int,long, np.float, np.float16, np.float32, np.float64, np.float128]) , "Reward should be a scalar with type long, or int, or long but " + str(type(r)))
    ...
    def test_evaluation(self):
        ...
        def checkEvaluation(env):
            ...
            print(J_eval, J)
            self.assertTrue(np.isclose(J_eval, J), "Two evaluation should be almost the same, we have instead: " + str(J_eval) + " " + str(J))
