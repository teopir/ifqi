...
rrr = rrr[:,[2,3,4,5,6,7,8,9,10, 14,15,16,17,18,19,20,21,22]]
kkl = kkl[:,[2,3,4,5,6,7,8,9,10, 13,14,15,16,17,18,19,20,21]]
titles = np.array(titles)[[2,3,4,5,6,7,8,9,10, 14,15,16,17,18,19,20,21,22]]
...
colors = ['r', 'b', 'g', 'y', 'm', 'k', 'c', '#FF9900', '#32CD32']

for i in range(9):
    y = rrr[:, i]
    y_upper = rrr[:, i+9] + y
    y_lower = y - rrr[:, i+9]
    ax.plot(_range, y, marker='o', color=colors[i], label=titles[i])
    #ax.errorbar(_range, y,
    #            yerr=[y_lower, y_upper],
    #            marker='+', label=comp_labels[i+2], color=colors[i])
    ax.fill_between(_range, y_lower, y_upper,
                    alpha=0.2, edgecolor=colors[i], facecolor=colors[i], antialiased=True)
...
ax.set_xlabel('#episodes')
ax.set_ylabel('kldiv')
fig.suptitle('REINFORCE - KLdiv - epsilon = %s' % epsilon)
