...
class AdvertisingMDP(Environment):
    ...
    def step(self, action, render=False):
        new_state = self.np_random.choice(3, p = self.P_sas[action])
        reward = self.R_sas[action,new_state]
    ...
    def computeQFunction(self, policy):
        return np.linalg.solve(np.eye(5) - self.gamma * np.dot(self.P_sas, policy), self.R_sa)
