...
class RFS(BaseEstimator, MetaEstimatorMixin, SelectorMixin):
    def __init__(self, feature_selector, verbose=0):
    ...
    def _recursive_step(self, X, next_state, Y, support):
        ...
        new_s_support[support[0:n_states]] = False  # remove features already selected
        idxs = np.where(new_s_support)[0]  # get numerical index

        # update support with features selected at this iteration
        sa_support[support] = True
