...
class Environment(gym.Env):
    ...
    def _eval_and_render(self, policy, nbEpisodes=1, metric='discounted', initialState=None, render=True):
        """
    ...
    def _parallel_eval(self, policy, nbEpisodes, metric, initialState):
        #TODO using joblib
        return self._eval_and_render(policy, nbEpisodes, metric, initialState, False)
    ...
    def evaluate(self, policy, nbEpisodes=1,
        ...
        if render:
            return self._eval_and_render(policy, nbEpisodes, metric, initialState, True)
        else:
            return self._parallel_eval(policy, nbEpisodes, metric, initialState)
