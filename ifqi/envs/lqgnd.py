...
class LQGND(gym.Env):
    ...
    def __init__(self, discrete_reward=False):
        ...
        self.B = np.array([[1., 0.1],
                           [0.1, 1.]])
        '''
        self.Q = 0.9 * np.eye(dim)
        self.R = 0.9 * np.eye(dim)
        ...
        self.action_space = spaces.Box(low=-self.max_action,
                                       ...
                                       shape=(dim,))
        self.observation_space = spaces.Box(low=-high, high=high, shape=(dim,))

        self.initial_states = np.array([[1, 2, 5, 7, 10], [0, 0, 0, 0, 0]]).T
    ...
    def step(self, action, render=False):
        ...
        if self.sigma_noise > 0:
            noise = self.np_random.randn(self.dim) * self.sigma_noise
        xn = np.clip(np.dot(self.A, self.state) + np.dot(self.B, u) + noise,
    ...
    def _computeP2(self, K):
        ...
        if np.array_equal(self.A, I) and np.array_equal(self.B, I):
            P = np.dot((self.Q + np.dot(K.T, np.dot(self.R, K))),
                       np.linalg.inv(I - self.gamma *
                                     (I + 2 * K + K **
                                      2)))
